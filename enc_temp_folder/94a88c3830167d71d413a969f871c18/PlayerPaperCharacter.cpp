...
void APlayerPaperCharacter::Tick(float DeltaTime) {
	...
	//TODO: Refactor firing direction code
...
void APlayerPaperCharacter::ShootToHeading(Heading HeadingDirection) {
	if (bCanFire == true) {
		...
		if (World != NULL) {
			...
			bCanFire = false;
			World->GetTimerManager().SetTimer(TimerHandle_ShotTimerExpired, this, &APlayerPaperCharacter::ShotTimerExpired, 1.f /*EquippedWeaponComponent->GetFireRate()*/);
		}
